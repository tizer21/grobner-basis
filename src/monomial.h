...
public:
	...
	TMonomial(std::initializer_list<TVariableDegree> initList)
	: Variables_(initList)
	{
	...
	TVariableDegree GetDegreeOf(TVariableIndex i) const {
		return (i < GetEndIndex() ? Variables_[i] : 0);
	}
	...
	TVariableIndex GetEndIndex() const {
	...
	reverse_iterator rbegin() {
		return Variables_.rbegin();
	}

	reverse_iterator rend() {
		return Variables_.rend();
	}
	...
	friend bool operator==(const TMonomial& lhs, const TMonomial& rhs) {
		return lhs.Variables_ == rhs.Variables_;
	}
	...
	static TMonomial Lcm(const TMonomial& lhs, const TMonomial& rhs) {
